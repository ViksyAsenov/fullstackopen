...
const authorWithMostBlogs = (blogs) => {
  ...
  let authorsAndBlogCount = blogs.reduce((authorAndBlogCount, blog) => {
    authorAndBlogCount[blog.author] = (authorAndBlogCount[blog.author] || 0) + 1

    return authorAndBlogCount
  }, {})

  let mostBlogs = Math.max(...Object.values(authorsAndBlogCount))
  let authorWithMostBlogs = Object.keys(authorsAndBlogCount).find(author => authorsAndBlogCount[author] === mostBlogs)
...
const authorWithMostLikes = (blogs) => {
  ...
  let authorsAndLikeCount = blogs.reduce((authorAndLikeCount, blog) => {
    authorAndLikeCount[blog.author] = (authorAndLikeCount[blog.author] || 0) + blog.likes

    return authorAndLikeCount
  }, {})

  let mostLikes = Math.max(...Object.values(authorsAndLikeCount))
  let authorWithMostLikes = Object.keys(authorsAndLikeCount).find(author => authorsAndLikeCount[author] === mostLikes)
